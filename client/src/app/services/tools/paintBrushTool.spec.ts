...
import { KeyboardEventType, MouseEventType, PrimitiveType, Texture } from '../utils/constantsAndEnums';
import { Point } from '../utils/point';
import { PaintBrushTool } from './paintBrushTool';

describe('PaintBrushTool', () => {
    let tool: PaintBrushTool = new PaintBrushTool(new Color(128, 64, 32, 0.5));
    const commandAttributeName = 'command';

    beforeEach(() => {
        tool = new PaintBrushTool(new Color(128, 64, 32, 0.5));
      });

    it('Beginning is correctly identified', () => {
        const expected: Path = new Path(new Color(128, 64, 32, 0.5), 5, PrimitiveType.Paint, Texture.Basic);
    ...
    it('Path properly finishes', () => {
        const expected: DrawingToolCommand = new DrawingToolCommand(
            new Color(128, 64, 32, 0.5), 5, PrimitiveType.Paint, Texture.Basic);
